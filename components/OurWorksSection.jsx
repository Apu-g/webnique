...
/* ── Manual line-split (replaces SplitText with mask) ── */
function splitIntoLines(el) {
  ...
  // Group by offsetTop to detect lines
  let top = null;
  const lineEls = [];
  ...
  // Remove orphaned word spans
  wordEls.forEach((sp) => sp.remove());

  const lineMasks = []; // These stay in place (the "mask")
  const splitLines = []; // These get animated (the "line" inside the mask)
  ...
  return splitLines; // Return the inner line elements (these get yPercent: 90)
}
...
export default function OurWorksSection() {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      sections.forEach((section) => {
        ...
        // Split heading into lines with masks (like SplitText)
        const lineEls = splitIntoLines(h2);

        // Set initial: each line starting 90% below
        gsap.set(lineEls, { yPercent: 90 });

        // Scroll-triggered background skew
        gsap.to(bg, {
          y: -220,
          skewY: -8,
          ease: 'none',
  ...
  return (
    <div className="ow-main" ref={wrapperRef}>
        ...
        {/* Work sections — same structure as creative-kai */}
        {WORKS.map((work, i) => {
